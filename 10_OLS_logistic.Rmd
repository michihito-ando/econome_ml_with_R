...
線形回帰でパラメータ$\boldsymbol{\beta}$（$=\begin{pmatrix}\beta_1 & \beta_2 & \cdots & \beta_p\end{pmatrix}$）を推定するために多く使われる方法は**最小二乗法**（least squares method）というもので，これは実測値$y_i$と予測値$\hat{y}_i$の**誤差二乗和**（Sum of Squared Error）
...
なお，$\argmin$という記号は「関数の最小値を与える引数」という意味である。$\min$と$\argmin$の関係は次の図のようになっている。

<center><img src="10_OLS_logistic.assets/1552122544346.png" width = 40%></center>

SSEは（パラメータが1つの場合）U字型のカーブを描く関数になる。

最小値が１つであり微分可能なので，推定量$\hat{\boldsymbol{\beta}}^{LS}​$を得るための計算が容易であるという便利な特性をもっている。
...
ggplot(data_frame(weight = w, loss = cost_vec),
  ...
  labs(title = "誤差二乗和（SSE）", y = "誤差ε", x = "パラメータβ")+
  theme_classic()+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank())
...
最小二乗法では，誤差を最小化するようなパラメータ$\beta_j^*$の値を推定するのが目的であり，それはU字型の誤差関数であるSSEの最小値であるので，次の条件（接線の傾きが0であること）を満たすパラメータとなる。

$$
\begin{cases}\frac{\partial SSE}{\partial \beta_0} = -2 \sum_{i=1}^n (y_i - \beta_0 - \beta_1 x_i) = 0\\\frac{\partial SSE}{\partial \beta_1} = -2 \sum_{i=1}^n (y_i - \beta_0 - \beta_1 x_i)x_i = 0\end{cases}
$$
...
ggplot(sse,
  ...
  geom_point(aes(x = sse[sse$loss == min(sse$loss),"weight"][[1]],
                 y = min(sse$loss)),
             color = "orange", size = 2)+
  labs(title = "最小二乗推定量", y = "誤差ε", x = "パラメータβ")+
  theme_classic()+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank())
...
\beta_1 &= \frac{\sum_{i=1}^n(x_i-\bar{x})(y_i-\bar{y_i})}
...
機械学習の目的は未知のデータに対する予測性能を高めることであるため，データを学習（パラメータの推定）に使う「学習用データ」と学習に使わず予測性能の試験に使う「テスト用データ」に分割しておく。
...
MSEの平方根をとることでこの点を修正したものが$RMSE$である。
$$
...
ロジスティック回帰の誤差関数は尤度関数の対数をとり負にした**交差エントロピー誤差関数**（cross-entropy error function）というものを使う。

交差エントロピー誤差は最小二乗法の平均二乗誤差とは異なり「微分してゼロになるパラメータを使う」というアプローチ（解析的に解く方法）はとれないため，**ニュートン・ラフソン法**（Newton-Raphson method）や**勾配降下法**（gradient descent method）を用いて，誤差関数を小さくするように徐々にパラメータを変化させていく計算を繰り返すアプローチをとる（数値計算的に解く）。
