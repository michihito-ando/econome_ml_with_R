...
線形回帰でパラメータ$\boldsymbol{\beta}=\begin{pmatrix}\beta_1 & \beta_2 & \cdots & \beta_m\end{pmatrix}^\top$を推定するために多く使われる方法は**最小二乗法**（OLS: ordinary least squares method）というもので、これは実測値$y_i$と予測値$\hat{y}_i$の二乗誤差$(y_i - \hat{y}_i)^2$の和による誤差関数$L(\boldsymbol{\beta})$
...
を採用するというものである（なお、$\argmin$という記号は「関数の最小値を与える引数」という意味である）。
...
例えば、あるコインを4回投げて表が1回出たとする。表が出る回数が二項分布に従うと仮定すると、このデータをもたらす確率が最も高い二項分布は、表が出る確率が「4分の1」の二項分布になる。そのため、このコインが表を出す確率の推定値として「4分の1」を採用する、というのが最尤法の考え方である。

より具体的には、コイン投げの結果が、コインの表が出る回数$k$が、表が出る確率$p$、試行回数$n$の二項分布

$$
Bin(k|n,p) = {}_n\mathrm{C}_kp^k(1-p)^{n-k}
$$

に従うと仮定し、4回投げて表が1回という結果（データ）を踏まえて、**尤度関数**（likelihood function）$L(p)$を
...
最尤法では一般に解析的に解が求まらないため、ロジスティック回帰の推定では**ニュートン・ラフソン法**（Newton-Raphson method）という方法を用いて数値的に解く。これは尤度が高くなる方向へ少しずつ推定値を変える計算を何度も反復することで推定する。


### （参考）対数尤度と交差エントロピー誤差{-}

確率の積は非常に小さな値になってしまうため、実際に計算する際は対数で変換した**対数尤度**（log-likelihood）を使用する。

また統計学の分野では最尤法という言葉を使うことが多いが、機械学習の分野では対数尤度の符号を反転させたもの

$$
\mathcal{L}(p)
= - \ln L(p)
= \sum^N_{i=1} \{ y_i \ln p + (1 - y_i) \ln (1 - p)\}
$$

を**交差エントロピー誤差**（cross entropy error）と呼ぶ。
