...
\begin{align}
L(p)
&= P(X = 1| p) \times P(X = 0| p) \times P(X = 0| p) \times P(X = 0| p)\\
&= p^1(1-p)^3
\end{align}
$$
...
ロジスティック回帰の推定では**ニュートン法**（Newton's method）と呼ばれるという方法を用いることが多い。ニュートン法では傾きの計算に$\mathcal{L}$の2次の導関数も用いて次のように推定値を更新する。

$$
\hat{\theta}_{t+1} = \hat{\theta}_{t}
- \frac
{ \mathcal{L}'(\hat{\theta}_{t}) }
{ \mathcal{L}''(\hat{\theta}_{t}) }
$$
...
for (i in 1:10) {
  p_prev <- p
  p <- p - (grad(p) / hess(p))  # pを更新

  # 結果をベクトルに保存
  p_vec <- append(p_vec, p)
  p_prev_vec <- append(p_prev_vec, p_prev)

  # 終了判定
  if (abs(p_prev - p) < e) break
}
...
estimates <- tibble(
  new = p_vec,
  prev = p_prev_vec,
  desc = p_vec < p_prev_vec,
  反復回数 = 1:length(p_vec) %>% as.factor()
) %>% 
  tidyr::pivot_longer(
    cols = c(-`反復回数`, -desc),
    names_to = "state",
  ...
  mutate(error = -log_likelihood(p))


# 描画用データ
p <- 1:100 * 0.01
df <- tibble(p, `L(p)` = -log_likelihood(p))

# plot
ggplot(estimates, aes(x = p, y = error))+
  geom_line(data=df, mapping=aes(x = p, y = `L(p)`),
            color = "dodgerblue") +
  geom_point(data = estimates %>% filter(state == "new"),
             mapping = aes(color=`反復回数`))+
  geom_line(mapping = aes(x = p, y = error, group=`反復回数`),
            data = estimates %>% na.omit() %>% filter(desc == T),
            arrow = arrow(length=unit(0.30, "cm"), ends="first", type = "open"))+
  geom_line(mapping = aes(x = p, y = error, group=`反復回数`),
            data = estimates %>% na.omit() %>% filter(desc == F),
            arrow = arrow(length=unit(0.30, "cm"), ends="last", type = "open"))+
    labs(title = "ニュートン法によるパラメータ更新", y = "Error")
```
