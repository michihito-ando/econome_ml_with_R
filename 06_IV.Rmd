...
データ生成過程は、以下のように設定できる。（難しく感じる人は、コピペでかまわな）
...
```{r}
reg_biased1 = lm(income ~ university, 
                 data = df)
reg_biased2 = lm(income ~ university + exemption, 
                 data = df)
...
```{r}
first_stage = lm(university ~ exemption, 
                 data = df)
...
```{r}
reduced_form = lm(income ~ exemption, 
                  data = df)
...
iv_reg_robust <- estimatr::iv_robust(income ~ university | exemption, # 被説明変数 ~ 説明変数 | 操作変数
                            data = df, 
                            se_type ="HC1") 
