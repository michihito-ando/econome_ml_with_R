...
TSLSでは、{ivpack}パッケージを使うことにより、簡単にロバスト標準誤差を計算できる。
...
```{r}
iv_reg_robust <- robust.se(iv_reg)
stargazer(iv_reg, iv_reg_robust, type = "text")
```

`university`の係数推定値については、ロバスト標準誤差のほうが若干大きくなっている。
