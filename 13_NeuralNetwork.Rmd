...
ニューロン間の結合部（シナプス）の結合の強さ（神経伝達物質の放出量）はそれぞれ異なり、よく使う神経回路はシナプス結合が強くなり、そうすることで学習が進行すると考えられている。
...
<details>
  ...
  <section style="background-color: #D7EEFF; padding: 10px;">
...
# ある単語に類似した単語のtop5（モデルの精度が悪いためかうまく機能しない）
...
を用いる。これは入力値$\boldsymbol{x}=(x_1, x_2, \dots, x_K)$を0から1の値かつ全クラスの合計が1になるように（つまり「クラス$i$の確率」として）変換する関数である。
...
Transformerはattentionという計算処理を用いるディープラーニングのアーキテクチャ（ネットワーク構造）である。
ChatGPTの内部で使われるGPTシリーズやGoogleのBERTなど多くの言語モデルで用いられている。


## Transformerの構造

Transformerもいくつか種類がある。理解しやすさのため、比較的シンプルな構造をしているGPTシリーズのTransformerを例にとって説明していく。
...
「**Layer Norm**」は学習中などに極端に大きい値にならないようにベクトルの各要素を正規化する（平均を引いたり標準偏差で割ったりなどする）処理である。

「**Feed Forward**」は2層のDNNである。ただし、入力の次元数よりも中間層の次元数（ユニット数）を大きくすることが多い。


## Attention

attentionは入力の単語埋め込み$\boldsymbol{x}$を、クエリ$q$、キー$k$、バリュー$v$という3つの変数やsoftmax関数で計算してベクトル$\boldsymbol{x}'$を再構築する。
なお、$q, k, v$は入力の単語埋め込み$\boldsymbol{x}_i$に重みを掛けて計算されたものである。この重みも学習によって決定される。

attentionは次のような流れで計算されていく。
まず、クエリとキーと埋め込みの次元数$d$を使って$s_{ij}$を計算する。

$$
s_{ij} = \frac{ \sum_{l=1}^d q_l k_l }{ \sqrt{d} }
$$

分子の$\sum_{l=1}^d q_l k_l$は線形代数において内積と呼ばれる計算で、2つのベクトル間の類似度の計算に使われることもある計算である。
これを$\sqrt{d}$で割って正規化することで、類似度のような値になっている。

::: card
**（参考）コサイン類似度**

2つの$d$次元ベクトル$\boldsymbol{a, b}$があるとき、その内積$a \cdot b = \sum a_i b_i$とベクトルの長さ（ノルム）$\|a\|=\sqrt{ \sum |a_i|^2 }$から計算される
...
続いて、$s_{ij}$のベクトル$\boldsymbol{s}_i = (s_{i1}, s_{i2}, \dots, s_{dp})$（$p$はトークンの最大長）をsoftmax関数にかけて、合計が1になるように正規化する。
...
最終的な出力はこの重み$\boldsymbol{a}_i$とバリュー$\boldsymbol{v}_l$の内積で計算される。

$$
x'_{ij} = \boldsymbol{a}_i \cdot \boldsymbol{v}_l = \sum^p_{j=1} a_{ij} v_{jl}
$$

::: card
**argmaxとの対比で解釈するsoftmax**

softmaxは足し合わせたら1になる。
似たものでargmaxがあり、これも足し合わせたら1になるが、入力のベクトルのうち最も値が高い要素が1をとり、ほかは全部0になる。

例えばベクトル$s$が

$$
\boldsymbol{s} = (1, 3, 5, 2, 0)
$$
...
$$
\begin{align}
\boldsymbol{a}^{soft} &= \text{softmax}(\boldsymbol{s}) = (0.02, 0.11, 0.83, 0.04, 0.01)
\\
\boldsymbol{a}^{arg} &= \text{argmax}(\boldsymbol{s}) = (0, 0, 1, 0, 0)
\end{align}
$$

別のベクトル$\boldsymbol{v}=(v_1, v_2, v_3, v_4, v_5)$と内積をとったとき、argmaxのほうは内積をとったベクトルの1つの要素だけを取り出す操作に相当する
...
プログラミングにおいて、キー・バリュー・ストアというデータ構造がある。Rだと名前付きのlistがそれに相当し、名前がキー、値がバリューである。

```{r}
kvs <- list(昨日=0.2, は=0.1, 雨=0.7, が=0.1, 降り=0.1, まし=0.1, た=0.1)
kvs["雨"]
```

argmaxの場合はキー・バリュー・ストアと同様のふるまいになる。

一般的なattentionはsoftmaxを使用するため、「attentionはキー・バリュー・ストアをsoftに実装しているもの」（「取り出されるかどうか」の$\{0, 1\}$ではなく実数値で重みをつけて取り出すもの）と解釈することもできる
:::


なお、「Masked Multi Self Attention」の他の語については、

- 「Masked」は未来の時点の情報を見せないように覆い隠す（要素をゼロで埋める）処理である
- 「Multi」はこのattentionが複数ある（multi-head attention）ことを示す
...
2. 並列計算の問題：multi-head部分は並列に計算できる
...
例えばrinna株式会社の[rinna/japanese-gpt2-medium · Hugging Face](https://huggingface.co/rinna/japanese-gpt2-medium)は日本語のデータを用いてGPT-2を学習したモデルである。Inference APIもあるためウェブサイト上で試すことができる。
...
こうした問題に対して、モデル内に学習された知識ではなくモデル外の知識を利用して回答文を生成する方法が考えられており、**Retrieval Augmentation**あるいは**Retrieval Augumented Generation（RAG）**と呼ばれる。
