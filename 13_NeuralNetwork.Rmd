...
<!-- 言語モデルを作成するにあたり多く使われてきたアルゴリズムがRNNとLSTMである。以下で概要を説明していく。 -->
...
勾配消失を防ぐための工夫を施した**LSTM（long short-term memory）**や**GRU（gated recurrent unit）**といった手法も存在する。
...
GoogleのBERTやChatGPT内部で使われるGPTシリーズなど、多くの言語モデルで用いられている。
...
パラメータ数が極めて多い言語モデル（「多い」に明確な基準はないが例えば「100億パラメータ以上」など）を**大規模言語モデル（large language model: LLM）**と呼ぶ。巨大テック企業を中心に各社様々なモデルを開発している。

<center>
  <img src="13_NeuralNetwork.assets/llm-timeline.png" width="80%"/>
  <br/>
  <span style="font-size: 0.8em;">
      図：100億パラメータ以上の言語モデルを発表日順に並べたもの。
...
伝統的な自然言語処理においては、教師あり学習を用いることが多かった。例えば機械翻訳のタスクにおいて、翻訳元の言語と翻訳先の言語（例えば英語とフランス語）のペアの文章を用意しておき、教師あり学習として学習させるような方法がとられていた。

Transformerを使用する初期の研究であるGPT-1では、文脈から次のトークンを予測する問題を解く方法を用いて学習を行った。教師ラベルとペアとなる文章ではなく、単に通常の文章（例えばWikipediaの文章）を用意すればよいため、データの規模を飛躍的に増加させることが可能になった。

このように教師ラベルがないデータの一部を教師ラベルとみなして教師あり学習を解く方法を**自己教師あり学習**（self-supervised learning）と呼ぶ。

また、解きたいタスクに応じてその都度モデルを1から学習させるのではなく、事前に学習させて汎用的な知識を備えた大規模なモデル（**基盤モデル foundation model**）を用意しておき、様々なタスクに対して小規模な教師ありデータで追加学習（fine-tuning）を行うというアプローチも採用されるようになった。
...
この現象は**創発（emergence）**や**創発的能力（emergent abilities）**などと呼ばれる。
...
<center>
  <section style="width: 90%">
    ...
    <br/>
    <span style="font-size: 0.8em;">
      図：縦軸にfew-shot learning（後述）での正解率、横軸に計算回数（浮動小数点演算数Floating-point OPerations: FLOPs）をとった図。左の図は多段階の四則演算をするタスク、中央の図は大学レベルの試験のタスク、右の図は文脈内の単語の意味を推定するタスクを表す。
      <br/>
...
[ChatGPT](https://chat.openai.com/)はOpenAIが提供しているチャットボットのサービスである。
人間と遜色ない自然な文章を生成することから話題になった。

GPT-3は大規模言語モデルとして登場当初から評価が高かったものの、質問に応答するというタスクに対しての精度であったり、暴力的な内容（「爆弾の作り方を教えて」など）に対する回答の回避などといったチャットボットとして要求されるふるまいは身につけていないため、チャットボットとして使うためには改良を行う必要があった。
...
2. プロンプトに対するモデルの複数の出力を人間がランク付けし、その順序を報酬モデルを学習させる
