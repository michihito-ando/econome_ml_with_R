...
- 感情分析：文章中のネガティブな表現やポジティブな表現を抽出し、全体としてポジティブかネガティブかを評価する
...
- 文章生成：指定された文章を生成したり、文章を要約したり、チャットボットとしての返答を生成したりなど
...
ある中間層の値は、入力$x_t$と重みを

$$
z_t = h \left( \sum^m_{j=1} w_j^{(in)} x_{j, t} + \sum w^{(hid)} z_{t-1} \right)
$$

$$
z_t = h \left( \sum^m_{j=1} w_j^{(in)} x_{j}^{(t)} + \sum w^{()} z^{(t-1)} \right)
$$
