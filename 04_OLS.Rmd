...
線形回帰のパラメータ$\beta_0, \beta_1$はデータ$X_i,Y_i$から推定する。最も有名で非常に強力な推定方法は**最小二乗法**（least squares method）であり，これは推定したパラメータ$\hat{\beta}_0, \hat{\beta}_1$からの予測値$\hat{Y}_i$の実測値$Y_i$との誤差二乗和$\sum_{i=1}^n (Y_i - \hat{Y}_i)^2$を最小にするパラメータを使用する。
...
```{r}
reg1 <- lm(formula = log(wage) ~ education, data = CPS1985)
```

対数変換をしなかった線形回帰の結果`reg`と今回の`reg1`を`stargazer()`に入れることで，まとめて結果表示できる。
...
```{r}
reg2 <- lm(formula = log(wage) ~ education + experience, data = CPS1985)
```
...
```{r}
reg3 <- lm(formula = log(wage) ~ education + experience + experience2, data = df)
```
