...
を解く方法のひとつに**最急降下法**（steepest descent method）あるいは**勾配降下法**（gradient descent method）と呼ばれるものがある。これは目的関数の1次の導関数である**勾配**（gradient）を用いて

$$
x_{m} = x_{m-1} - \alpha_{m-1} \frac{\partial f(x_{m-1})}{\partial x_{m-1}}
$$

という値の更新を何度も繰り返して最適化を行っていく。（$\alpha$は学習率と呼ばれるパラメータで、値の更新量が多すぎると最適解を通り過ぎてしまうことがあるので小さめの値を乗じて更新幅を抑えるために用いられる）
...
ブースティングは最急降下法を機械学習で行っていると捉えることができる。
...
を解きたいため、勾配は誤差関数の予測モデルによる微分$\frac{ \partial L(y, f(x)) }{ \partial f(x) }$ということになる。
...
$$
L(y_i, f_{m-1}(x_i) + \beta f(x_i; \theta)) = (\text{residual}_{im} - \beta f(x_i; \theta))^2
$$

これにより、学習されるモデル$\beta f(x; \theta)$は負の勾配を学習するようになり、最終的にそれらの和となるモデルは最急降下法を解いた状態を近似することになる。
