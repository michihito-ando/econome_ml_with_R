...
pacman::p_load(
  ...
  DiagrammeR, # xgboostのplotのために使われるグラフ描画ライブラリ
  xgboost, # GBDTのライブラリ
  lightgbm # GBDTのライブラリ
)
```
...
である。
ベルヌーイ分布の分散は$p(1-p)$であるため、ジニ係数は数式の形としては各クラスごとの出現率の分散を計算して総和をとったような形となっている。
...
# plot
tibble(gini_range, p_range) %>% ggplot(aes(x = p_range, y = gini_range)) +
...
$$
\begin{align}
I(t) &= 0.5 \times (1 - 0.5) + 0.5 \times (1 - 0.5)\\
&= (0.25 + 0.25) = 0.5
\end{align}
$$
...
予測値は`predict()`で算出できる。`type = "class"`とすると目的変数のクラスがfactor型で出力され、`type = "prob"`だと各クラスの予測確率値が出力される。
...
ggplot(errors, aes(x=1:nrow(errors), y=OOB)) +
  geom_line(color="SteelBlue") +
  labs(y = "OOB誤分類率", x = "木の数", title = "ランダムフォレストの木の数と誤分類率") +
  theme_classic()
```
...
```{r}
# 予測
y_pred <- predict(titanic_RF, test, type = "class")
...
ブースティングでは複数の予測モデルを用いて逐次的に学習を行い、$m$回目の学習ではその1つ前の$m-1$回目に学習した予測モデルで予測誤差が多かったサンプルを重視する、というようなアプローチで学習していく。
...
を解く方法のひとつに**勾配降下法**（gradient descent method）あるいは**最急降下法**（steepest descent method）と呼ばれるものがある。これは目的関数の1次の導関数を用いて
...
（ちなみに名前の由来である**勾配**（gradient）とは偏微分のベクトル$\nabla f(\boldsymbol{w}) = \left( \frac{\partial f(\boldsymbol{w})}{\partial w_1}, \dots, \frac{\partial f(\boldsymbol{w})}{\partial w_n} \right)$のことである）


<center>
  <img src="12_Tree.assets/gradient_descent.png" width="40%"/>
  <br/>
  <span style="font-size: 0.8em;">図：勾配降下法のイメージ</span>
</center>
...
GBDT）**は上記のようなブースティングによる加法モデルの構築を決定木$T(x; \theta)$を用いて行うアルゴリズムである。
...
勾配ブースティング決定木は表形式データ（tabular data）に対して高い性能を発揮することが多く、頻繁に利用される。
有名な勾配ブースティング決定木のアルゴリズムにXGBoostとLightGBMがあり、Rからも利用可能なのでそれらを紹介していく。


## XGBoost

[XGBoost](https://github.com/dmlc/xgboost)（Chen & Guestrin,
...
は計算が非常に高速で大規模データとの相性が良い勾配ブースティング決定木のアルゴリズムである。
...
- 論文：[Breiman, L. (2001). Random forests. *Machine learning*, *45*(1),
...
- パッケージ：[Light Gradient Boosting Machine •
...
- 濵田晃一(2010)「[はじめてでもわかる RandomForest入門－集団学習による分類・予測－](https://www.slideshare.net/hamadakoichi/randomforest-web)」
