...
その後研究が進み、これらの問題への対処法が生み出されていった。

1. 勾配消失 → **ReLU**（rectified linear unit、活性化関数に正規化線形関数$f(x) = \max(x, 0)$を使ったユニット）などの提案
2. 過学習 → **Dropout**（中間層のニューロンを学習のたびにいくつかランダムに無効化しつつ学習する）などの正則化の手法
...
```{r}
# 予測値
fitted %>% predict(test_dl) %>% print(n=10)
```
