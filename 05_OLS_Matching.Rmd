...
<center><img src="05_Test_OLS_Matching.assets/05_causal_effect.png" width=70%></center>
...
ただし、ここで「大卒になることで、所得が`r round(mean(grad_df$income),0)` - `r round(mean(not_grad_df$income),0)` = `r round(mean(grad_df$income),0) - round(mean(not_grad_df$income), 0)`万円上昇する」と結論付けることはできない。例えば、学歴と所得の両方に影響を与える交絡要因がある場合、大卒と非大卒の所得差には、このような交絡要因の影響が反映されてしまう。
...
$\text{ATT}$は**処置群における平均処置効果**（average treatment effect on the treated）という。話を単純化するために処置効果が処置群と対照群で平均的に等しい（$\text{ATT}=\text{ATE}$）と仮定すると、上記の単純な差による推定量は$\text{ATE} + \text{Selection Bias}$であり、セレクション・バイアスの分だけATEから乖離する推定量であることがわかる。
...
しかし、経済学においては実験によって得られたデータを扱うことは稀であり、基本的にこの方法を利用することはできない。
...
<center><img src="05_Test_OLS_Matching.assets/05_fig1.png" width=50%></center>
