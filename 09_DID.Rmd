---
title: "Rで学ぶ計量経済学と機械学習 9<br> <br> 計量経済学5：差分の差分(DID)法"
...
<!-- pacman::p_load(lmtest) # lmやplmでロバスト標準誤差に修正（参考) -->


# データの生成

パネルデータ分析の応用として、**差分の差分法（difference in differences: DID）法**がある。
...
二期間DIDなので、１年目と２年目のデータ生成過程（DGP: Data Generating Process）をそれぞれ作成する。
...
df_panel_mean <- df_panel %>% summarize(
  income = mean(income),
  .by = c("treated", "year")  # すべての列（変数）でグループごとに平均値を計算
)
```


（なお、別の書き方として`group_by()`してから`summarize()`する方法もある）
...
<!--
  MEMO:
  dplyr::summarise_each(list(mean)) はdeprecatedなので書き換えた.
  summarise_each(list(mean))に相当する処理（全カラムでmean）は summarize(across(everything(), mean)) だが
  今回はincomeだけmeanがとれればよいので summarize(income = mean(income), .groups = "keep")にする
-->
...
$$
Y_{it} = \mu + \theta D_i + \pi T_t + \beta D_iT_t + \varepsilon_{it}
\tag{1}
$$
...
$$
\Delta Y_i =  \pi + \beta D_i + \Delta \varepsilon_{it}
\tag{2}
$$
...
`lm`を用いて、(1)式および(2)式に基づいてDID推定を行う。ただし標準誤差は均一分散を仮定して算出されている点に注意する。
...
この場合、個体ダミーを$n-1$個用いて推定してもよいが、`lm_robust`関数で個体固定効果モデルを推定してもいい。
...
<!-- 
ID=1万の個体固定効果なので、やや重い処理。
RStudio cloudがどうしても止まってしまうならサブサンプリングでなんとかする方法がある

df_temp <- tibble(ID = sample(1:n, size=1000))
df_selected = inner_join(df_panel, df_temp, by = "ID")
-->
...
- 安藤道人「計量経済学２」あるいは「計量経済特論２」の「差分の差分法（差の差法）」の[講義資料](https://sites.google.com/site/michihito7ando/lectures)

- [計量経済学応用 差分の差分法](https://yukiyanai.github.io/jp/classes/econometrics2/contents/R/difference-in-differences.html) (矢内勇生氏の講義ノート)

- [Chapter 18 Difference-in-Differences](https://theeffectbook.net/ch-DifferenceinDifference.html) (Nick Huntington-Klein (2021)"The Effect: An Introuction to Research Design and Causality")

- [Chapter 8 Difference-in-Differences](https://mixtape.scunning.com/difference-in-differences.html) (Scott Cunningham (2021) "Causal Inference: The Mixtape)
