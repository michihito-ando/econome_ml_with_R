...
つまり、パネルデータであれば、(1)式の処置群ダミー$\theta D_i$の代わりに、個体ダミーを用いてもよいし、そのほうが標準誤差は小さくなる。（ただし、繰り返しクロスセクションデータを用いたDID推定では個体ダミーは用いることはできない点は留意が必要である。）

この場合、`lm`関数で個体ダミーをn-1個用いて推定してよいが、`plm`関数で個体固定効果モデルを推定すればよい。
...
DID_plm1 <- plm(income ~ year + treated*year, data = df_panel, model = "within")
...
(2)式の一階の階差によるDIDと、推定値と標準誤差は完全に一致している。
